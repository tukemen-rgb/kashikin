...
YEARS = {14:('令和元年度',2019), 15:('令和2年度',2020), 16:('令和3年度',2021),
         17:('令和4年度',2022), 18:('令和5年度',2023), 19:('令和6年度',2024),
         20:('令和7年度',2025)}
...
def parse_exam(kai):
    ...
    for page in reader.pages:
        ...
        for s in pre:
            g = STMT.match(s)
            if g and g.group(1) not in stmts:
...
if __name__ == '__main__':
    db = []
    for kai, (era, year) in YEARS.items():
        qs = parse_exam(kai)
        for n in range(1, 51):
            q = qs[n]
            db.append(dict(id=f'R{kai}-{n:02d}', kai=kai, era=era, year=year, num=n,
